...
| A-206 | `void connect(SessionId id)` | `SESS_STATE_INVALID` | `session.stateChanged(connecting)` |
...
| A-212 | `ProbeResult probePort(ConnectParams params)` | `SESS_PARAM_INVALID` | `session.probeDone` |
...
`ElementType`：`waveform|bar`（D-52 收敛）。场景 JSON 符合 `visual-scene.schema.json`。
...
## 9. DevicePort 接口（Core 层，L2-001）

> 跨平台设备抽象接口，由 Domain 定义、Core 实现（SerialWorker / NetworkTransport）。测试可用 mock 替换（15.5）。UI/Service 不直接调用，经会话 connect/disconnect 间接使用。
...
实现：串口用 `SerialWorker`，网络用 `NetworkTransport`（见 13.3 Core 层组件）。
