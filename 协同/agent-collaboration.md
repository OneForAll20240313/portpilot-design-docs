...
| 新 Agent 接入与里程碑闸门 | 本文档第七章 | ✅ 本次产出 |
| gh 扫描脚本 | 附录 A（本文档内） | ✅ 本次产出 |

---

## 七、新 Agent 接入引导与里程碑闸门

### 7.1 多 Agent 并发共创（如何让多个 agent 一起干）

- **凭证复用**：多个 Dev agent 可复用同一组织 `GH_TOKEN`（`OneForAll20240313`）。凭证只用于读写组织仓库与看板，多个会话共享同一 token 不冲突——因为**任务互斥靠看板 `assignee` 加锁，不靠 token 区分身份**。
- **身份区分**：当多个 Dev 复用同一 token 认领任务时，`assignee` 会写成同一账号。为避免分不清谁在干哪个任务，**每个 Dev 会话必须用 `AGENT_ROLE=dev:<会话名>` 声明自己的唯一标识**，并在认领时把该标识写入任务 Issue 评论（如「`@<会话名>` 认领」），PM 据此区分各 Dev 的进度。
- **并发安全**：认领前先查 `assignee` 为空才认领；`pull --rebase` 解决代码冲突；契约文件由 PM 统一维护、Dev 只读。并发上限由【待研发】列的可认领任务数决定，Dev 认领完空任务即停，不重复抢。

### 7.2 里程碑闸门（Milestone Gate）

- **目的**：保证每个里程碑达成后先停下，由**用户（项目维护者）验收**，验收通过后才进入下一里程碑，避免一口气堆完导致不可控。
- **判定「M1 达成」**：M1 里程碑下**全部任务**（当前 9 个：#1 父 + #16/#17/#18 分层子任务 + #33/#34/#35 需求 + #36/#37 研发）均进入「已验收」或「已关闭」状态，且每条验收证据（测试报告 + CI run）已附。
- **闸门规则**：
  1. 所有 Dev agent 只认领**当前里程碑**（M1）的任务；任一任务属于 M2 起，一律不认领。
  2. PM 扫描确认 M1 全部任务满足验收后，**停止调度**，产出《M1 验收报告》（落盘 `协同/进度/`），并通知用户验收。
  3. 用户验收通过前，任何 agent 不得认领/启动 M2 任务；用户确认后，PM 才解锁 M2。
- **解锁方式**：用户验收通过后，PM 在看板把 M2 任务从受保护状态放开（或由用户明确指令），Dev 方可继续认领 M2。

### 7.3 新 Dev Agent 接入清单（Step-by-Step）

> 任何新 agent 进入会话，按此清单自助接入，无需依赖其他 agent 的私有上下文。

1. **读文档（身份确认）**：读文档仓《项目共创准则.md》第八章 + 《协同/agent-collaboration.md》+ 代码仓 `PortPilot/README.md`，确认自己是 Dev、边界是"认领/实现/测试/PR/移列"。
2. **声明身份**：确认 `AGENT_ROLE` 已注入且非只读；否则仅只读巡检。
3. **看板定位**：`GH_TOKEN=xxx` 用 gh 扫描看板项目6，只筛选**当前里程碑（M1）** 且【待研发】列、`assignee` 为空的任务。
4. **认领（加锁）**：`gh issue edit $NUM --add-assignee @me` + `gh project item-edit` 移「研发中」，回读校验，评论写「`@<会话名>` 认领」。
5. **实现**：`git pull --rebase` → 按契约实现 → 编译通过 → 单测通过 → subagent 评审 7 维。
6. **交付**：`feat/` 分支 → PR → 等 CI 通过 → 任务移「待验收」+ 附结论/测试报告/CI 链接。
7. **停下检查**：完成当前任务后，若 M1 未全部验收，回到第 3 步认领下一个；若 M1 已全部验收，**停下等用户验收**，不认领 M2。
